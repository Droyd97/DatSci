...
//TODO: Integrate into another file
internal enum MatrixError: Error{
  ...
  case invalidSubtraction(got: MatrixDimension, expected: MatrixDimension)
}

internal func enforceAddDimensions(got: MatrixDimension,
                                   expected: MatrixDimension) throws {
  guard got == expected else {
    throw MatrixError.invalidAddition(got:got, expected:expected)
  }
}

internal func enforceSubtractDimensions(got: MatrixDimension,
                                        expected: MatrixDimension) throws {
  guard got == expected else {
    throw MatrixError.invalidSubtraction(got: got, expected: expected)
  }
}
